...
%The simulation will be animated Animate as it runs, and when finished the
%plot corresponding to Figure 2 for the particular simulation will appear in a new plot window. 

%To run one simulation specify model type M (0: Attr+Rep only. 
% 1: Attr+Rep+Alignment), speed difference s (1,2,3 or 4).

%The output U contains the simulation and the simulation can be rewatch
%using the matlab command implay(U).

% Examples
% The command FishP(0,2) will run the Attr+Rep only model (M=0) with speed
% difference 2 (s=2).

% The command Fish(1,3) will run the Attr+Rep+Alignment model (M=1) with
% speed difference 3 (s=3).

F=2; %Set to collect trajectories over time to create Figure 2 plot for current simulation.
...
kk=1; %Index for animation
%PHI=[];

%Cell array to collect particle positions through time
X=cell(N,1);

S=0; %Initiate speed check condition

%Check that valid speed input
if s~=1 && s~=2 && s~=3 && s~=4
    disp('Not valid speed difference s, choose s to be 1,2,3 or 4')
    S=1;
end

if S==0 %If valid speed input

P(:,5)=ones(N,1); %Initiate speed difference from seeing predator (at start no particle has seen predator so s=1 for all)

for k=1:t %For every timestep
       
        PP=[zeros(N,4),P(:,5)]; %To store updated positions in this timestep.
...
%         end
      ...
      hold off
        ...
        P=PP; %Update population matrix P by assigning it to be PP which contains updated positions from this timestep.
end
...
%CREATE FIGURE 2 FOR CURRENT SIMULATION (see mkFig2.m)
figure;
DXs1=plotW(X);
D=DXs1;
for i=1:N
