...
% 1: Attr+Rep+Alignment), speed difference s, and which Figure the
%simulation is for (2: Figure 2. 3: Figure 3.).

% Depending on which Figure the simulation is for the output will be different.
% If F=2 (Figure 2) the output Y will be the coordinates of each fish throughout the 
% whole simulation, contained in a cell array caled X in the code.
% If F=3 (Figure 3) the output will be the Instantaneous alignment \phi
% throughout the whole simulation, contained in a vector called PHI in the
% code.

% Examples
% The command Fish(0,2,3) will run the Attr+Rep only model (M=0) with speed
% difference 2 (s=2) and return the instantaneous alignment for Figure 3 (F=3)
% over the simulation.
% The command Fish(1,3,2) will run the Attr+Rep+Alignment model (M01) with
% speed difference 3 (s=3) and return the particle positions for Figure 2
% (F=2) over the simulation.
