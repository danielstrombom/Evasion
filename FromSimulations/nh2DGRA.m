function CMAL=nh2DGRA(i,P,R,Rr,b)
%Function for calculating the neighbourhood of particle i, the direction of
%the local center of mass and repulsive force AND ALIGNMENT

ba=b/2; %Half the blind angle
N=size(P,1); %Number of particles
NN=zeros(N,4); %To collect nearest neighbors within the attraction radius
NR=zeros(N,4); %To collect nearest neighbours within repulsion radius
count=0;
count2=0;
    ...
    if j~=i %non-self particle
        if (P(i,1)-P(j,1))^2+(P(i,2)-P(j,2))^2<R^2 %If particle j within distance R from particle i
            ...
            if abs(theta)>ba %If particle j not in i:s blind angle add as neighbor
                count=count+1;
                NN(count,:)=[P(j,1),P(j,2),P(j,3),P(j,4)];
            end
            if (P(i,1)-P(j,1))^2+(P(i,2)-P(j,2))^2<Rr^2
                count2=count2+1;
                NR(count2,:)=[P(j,1),P(j,2),P(j,3),P(j,4)];
            end
        end
    end
end
